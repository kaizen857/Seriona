...
- 构造时注入执行器（`SERIONA_HAS_BACKEND` 分支内共 10 个：playback/library 命令、文件夹排序、扫描，以及 SettingsController 的输出组 apply、播放过渡组 apply、均衡器组 apply、设备枚举、日志级别、歌词切分目标语言），连接播放开始回调（`trackStarted` → `m_trackStats.recordPlayback` 自增播放计数）、2 类快照信号（playerSnapshotChanged/librarySnapshotChanged）、均衡器/频谱/歌词三路订阅信号（equalizerStateChanged/spectrumChanged → SettingsController 只读镜像面，trackLyricsChanged → LyricsModel）、1 类域通知信号（domainNotificationQueued，含 FolderSortRulesApplied 应用与 ConfigureOutput 拒绝回退两支），另连接 `WaveformProvider::waveformReady`（`m_shuttingDown` 时丢弃）；`backendBridgeAutostartEnabled()` 读取 `QCoreApplication` 动态属性 `seriona.backendBridgeAutostartEnabled`（默认 true）决定是否自动 `BackendBridge::start()`。
- `shutdown()` 幂等；析构与 `QCoreApplication::aboutToQuit`（DirectConnection）均触发。
- Q_INVOKABLE：`shutdown()`、`scanLibrary(QUrl)`、`restorePlaylistFromStartup()`；曲目/队列命令入口 `deleteTarget(path, folder)`、`playNextTrack(trackId)`、`removeFromQueue(queueIndex)`（后两者与删除链消费命令结果的 `accepted` 并本地 toast，mock-only 下走 `NotificationController::showUnsupportedAction`）、歌词行级纠错入口 `upsertLyricSplitCorrection(rawText, original, translation)`、`removeLyricSplitCorrection(rawText)`（前三项同款：经 `BackendBridge` 外发控制命令）、整首纠错窗口入口 `lyricSplitBoundaryParts(rawLine, boundaryIndex)`（拖动分界→(原文, 译文) 的纯换算，供实时预览）与 `commitLyricSplitBoundary(rawLine, boundaryIndex, currentOriginal, currentTranslation)`（换算后与当前展示值相同则视为无操作、不发命令）与只读查询 `filePathForNodeId(nodeId)`；测试钩子：`backendBridgeStartedForTests()`、`backendNotificationCountForTests()` 等。
...
- 8 角色：rawLine/displayLine/translation/isCurrent/timestampSec，以及行级纠错用的 manualOverride/autoOriginal/autoTranslation（被 manual 覆盖前的自动判定结果，供 D14 菜单第 4 项只读展示）；`Qt::DisplayRole` 与 displayLine 等价。
- 数据来源是控制层 `TrackLyricsSnapshot` 订阅（BackendBridge 第 6 路订阅 → `AppFacade::handleTrackLyricsChanged`）：每行的原文/译文直接取后端给出的 `SplitLyricLine.original`/`translation`，前端只做原样渲染 + 行推进，不按分隔符切分，也没有兜底切分分支。`showTranslation` 属性控制翻译显示，`toggleTranslation()`/`selectLyric()` 为 QML 交互入口；`setPlaybackPosition` 同步当前行（最后一条 timestamp ≤ 位置）；无时间戳歌词恒指向第 0 行。行级手工纠错经 `appFacade.upsertLyricSplitCorrection`/`removeLyricSplitCorrection`（`AppFacade` Q_INVOKABLE → `BackendBridge` 外发 `UpsertLyricSplitCorrection`/`RemoveLyricSplitCorrection`，约定取自当前快照），前端不直连 DB。
- 整份替换 + 内容去重：`hasTimedLyrics` + 行数 + 逐行 timestamp/text/original/translation/manualOverride/autoOriginal/autoTranslation 全等则只重新同步当前行索引，不 reset、不清空；内容不同才 `beginResetModel` 重建。同一曲目的空行回退快照（后端在无当前曲目/找不到曲目/清洗后无正文行/store 失败时发布，trackId 仍为当前曲目）清空既有行，不保留上一首的行。
...
- **MainContent.qml**（1899 行）：播放/歌词双 state 共享元素迁移（400ms InOutCubic）；播放控制条、音量、进度（波形拖拽 seek、歌词态线性滑杆）、封面三层回退（全图 `coverArtworkSource` → 缩略图 `coverThumbnailSource` → 占位符"🎵"，逐层降级）、设置 BubbleMenu（"设置"→`openSettingsRequested` 打开 SettingsWindow，歌词跟随恢复延迟等真实设置项在窗口内；"均衡器"→`openEqualizerRequested` 打开 EqualizerWindow；"关于 Seriona"→AboutOverlay 真实关于界面；"整首纠错"→`openLyricSplitEditorRequested` 打开 LyricSplitEditorWindow；退出→真实关闭）、歌词行右键菜单（`LyricLineContextMenu`，D14 四项：修正原文/译文、此行无译文、恢复本行自动识别走控制命令，查看本行自动判定为只读）、通知 toast（3200ms 自动隐藏）。
...
BackendBridge::librarySnapshotChanged ──► AppFacade::handleLibrarySnapshotChanged
   ...
   ├─►（重复上述两条投影，library 侧 forceReapply=true）
   └─► WaveformProvider.requestForSnapshots(...)

BackendBridge::trackLyricsChanged ──► AppFacade::handleTrackLyricsChanged ──► LyricsModel.applyTrackLyricsSnapshot(trackLyricsSnapshot)
...
| 歌词 | 控制层 `TrackLyricsSnapshot` 订阅（后端切分）→ 原样渲染 → `playbackPosition` 绑定驱动当前行 |
